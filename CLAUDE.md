...
`.github/workflows/update-github-stats.yml` runs manually, daily, and on changes to `main` other than the generated snapshot. It supplies `secrets.GH_USERNAME` and `secrets.GH_TOKEN` (or, optionally, `secrets.GH_PAT` mapped to `GH_TOKEN`), allows `contents: write`, and commits an updated `src/data/github-stats.json`. The generated snapshot is public display data; secrets must remain workflow/environment inputs and must not be imported by frontend code. Secret names must not begin with `GITHUB_`.
