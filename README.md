...
npm install
cp .env.example .env
# điền GITHUB_USERNAME và GITHUB_TOKEN trong .env
...
Trong repository, mở **Settings → Secrets and variables → Actions → New repository secret** và tạo `GITHUB_USERNAME` với username GitHub cần theo dõi. Workflow sử dụng `secrets.GITHUB_TOKEN` do GitHub tự cấp; token này chỉ tồn tại trong runner và không được nhúng vào bundle frontend, nên không cần tạo secret token thủ công. Nếu cần hạn mức riêng, hãy lưu Personal Access Token dưới tên `GH_PAT` rồi đổi mapping trong workflow thành `GITHUB_TOKEN: ${{ secrets.GH_PAT }}`. Token local trong `.env` cũng chỉ được đọc ở script data pipeline.
